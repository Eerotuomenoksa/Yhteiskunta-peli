...
- 🏫 Koulusimulaatio (vk3 pe): kestävyystarkistus hitaalle yhteydelle, halvalle koneelle ja verkkokatkolle, tulokset [KOULUSIMULAATIO.md](KOULUSIMULAATIO.md):ssä. Peli ja testaajat.html vahvistettu toimivan täysin ilman verkkoa (jsdom-testi, `fetch()` epäonnistuu aina — 0 kaatumista, laskurit/bugilista näyttävät selkeän virhetilan). Merkittävin löydös: testaajien käyttämä `game/yhteiskunta.html` siirtää verkon yli n. 824 kt CDN-riippuvuuksia (josta 655 kt on pelkkää @babel/standalone-kääntäjää) — 3,5–4× enemmän kuin jo olemassa oleva Vite-bundle (227 kt), jota ei ollut vielä julkaistu missään testaajien tavoitettavissa.
- 🚀 Vite-bundle julkaistu GitHub Pagesiin (`play/`-kansio, käyttäjän vahvistuksella koulusimulaation löydöksen jälkeen): `vite.config.js`:n `outDir` vaihdettu gitignoretusta `dist/`:stä committoitavaan `play/`:hyn — GitHub Pages (legacy "deploy from branch" -tila) tarjoilee sen automaattisesti osoitteesta `/play/` ilman erillistä julkaisuputkea. `testaajat.html`:n ja markkinointisivun "Pelaa peliä" -linkit vaihdettu osoittamaan tänne (n. 4× kevyempi lataus); `game/yhteiskunta.html` säilyy toissijaisena "lataa offline-versio" -linkkinä.
- 📘 Opettajan oppaan ensiversio ([OPETTAJAN_OPAS.md](OPETTAJAN_OPAS.md), D7, vk4 ma): pelin pedagoginen idea, kytkennät LOPS 2021:n (HI1, HI2, HI6, YH1, YH2) ja POPS 2014:n (vuosiluokat 7–9, historia) sisältöihin, 45 min ja 75 min tuntisuunnitelmat (mitoitettu oikealla pelin etenemisnopeussimulaatiolla), esitystilan ja telemetrian käyttöohjeet opettajalle, tekniset vaatimukset.
