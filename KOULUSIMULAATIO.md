...
**Ratkaistu 13.7.:** käyttäjä vahvisti julkaisun. `vite.config.js`:n `outDir` vaihdettiin gitignoretusta `../dist`:stä committoitavaan `../play`:hyn. GitHub Pages on tässä projektissa "deploy from branch" -tilassa (tarjoilee `main`-haaran tiedostot suoraan, ei erillistä Actions-julkaisua) — koska `play/` on nyt osa gitiä, se on automaattisesti tavoitettavissa osoitteesta `/play/` ilman mitään Pages-asetusmuutoksia. `testaajat.html`:n ja markkinointisivun (`index.html`) "Pelaa peliä" -linkit vaihdettu osoittamaan sinne; `game/yhteiskunta.html` säilyy toissijaisena "lataa offline-versio koneelle" -linkkinä. **Muistettava jatkossa:** `npm run build` + `play/`-kansion committointi aina kun `game/yhteiskunta.jsx` tai `data/tietolaatikot.mjs` muuttuu, muuten julkaistu versio jää jälkeen lähdekoodista.
