...
export default function Yhteiskunta() {
  ...
  const professions = useMemo(() => {
    ...
    return [
      { name: "Viljelijät ja karjanhoitajat", n: w("farm"), note: `1 ruokkii ${Math.round(era.feeds * M.farm * derived.efficiency * derived.skillPenalty * derived.farmMode.mods.feedsMult)}` },
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        {s.civ ? (
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 8, marginBottom: 16, paddingBottom: 14, borderBottom: "1px solid #263a54" }}>
            <div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(20px,3.2vw,30px)", fontWeight: 700, letterSpacing: 0.5, color: "#f0e8d2" }}>
                ...
                {s.nation ? `${s.nation.icon} ${s.nation.name}` : `${s.civ.icon} ${s.civ.name}`}
              </div>
              <div style={{ fontSize: 13, color: "#93a3ba", letterSpacing: 2, textTransform: "uppercase", marginTop: 2 }}>
                {era.name} aikakausi · Vuosi {s.year}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: "#93a3ba" }}>Sivilisaatioindeksi</div>
          ...
          <header style={{ textAlign: "center", marginBottom: 16, borderBottom: "1px solid #263a54", paddingBottom: 14 }}>
            ...
            <div style={{ color: "#93a3ba", marginTop: 4, fontSize: 14 }}>
              Society · 10 000 ihmistä. Ei tuontia. Ei vientiä. Kuinka pitkälle pääsette?
            </div>
          </header>
        )}
              ...
              {CIVS.map((c) => {
                ...
                return (
                  <button key={c.key} className="civcard" onClick={() => setPendingCiv(c)}>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 700, color: "#f0e8d2" }}>{c.icon} {c.name}</div>
                    <div style={{ fontSize: 12, color: "#c9a227", fontWeight: 600, margin: "2px 0 4px" }}>{c.trait}</div>
                    <div style={{ fontSize: 12, color: "#cfd6de", marginBottom: 6 }}>{c.desc}</div>
                    {mods.map(([k, v]) => (
        ...
        {!s.civ && pendingCiv && (
          <div style={{ maxWidth: 640, margin: "0 auto" }}>
            <button onClick={() => { setPendingCiv(null); setChosenSkills([]); }} style={{ background: "none", border: "none", color: "#c9a227", fontSize: 13, cursor: "pointer", padding: 0, marginBottom: 12 }}>◂ Vaihda sivilisaatio</button>
            <div className="paper" style={{ marginBottom: 14 }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 700, marginBottom: 10, color: "#f0e8d2" }}>{pendingCiv.icon} {pendingCiv.name} — nimeä kylläsi</div>
              <input
              ...
              />
              <p style={{ fontSize: 12, color: "#93a3ba", margin: "6px 0 0" }}>Nimi näkyy tarinassa ja historianäkymässä — hyvä nimi tekee myöhemmästä esittelystä muille hauskemman.</p>
            </div>

            <div className="paper">
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 700, marginBottom: 4, color: "#f0e8d2" }}>Valitse kaksi aloitustaitoa</div>
              <p style={{ fontSize: 12.5, color: "#93a3ba", margin: "0 0 10px" }}>Nämä ovat pieniä pysyviä bonuksia koko peliajaksi — valitse ne, jotka sopivat suunnittelemaasi strategiaan. ({chosenSkills.length}/2 valittu)</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(230px,1fr))", gap: 8 }}>
                {STARTING_SKILLS.map((sk) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 15, fontWeight: 700, color: "#f0e8d2" }}>{sk.icon} {sk.name}{active && " ✓"}</div>
                      <div style={{ fontSize: 11.5, color: "#cfd6de", marginTop: 2 }}>{sk.desc}</div>
                    </button>
                  );
                })}
              </div>
        ...
        {s.civ && (
          <>
            <div className="stat-grid">
              <Stat label="Väkiluku" value={fmt(pop)} sub={`👶${fmt(s.children)} 💪${fmt(s.workers)} 👴${fmt(s.elderly)}`} warn={pop < 2000} />
              <Stat label="Ruokavarasto" value={fmt(s.foodStore)} sub={`Tase ${foodBalance >= 0 ? "+" : ""}${fmt(foodBalance)}`} warn={foodBalance < 0 && s.foodStore < pop * 0.5} />
              <Stat label="Tutkimus" value={nextEra ? `${fmt(s.research)}/${fmt(nextEra.research)}` : fmt(s.research)} sub={nextEra ? `→ ${nextEra.name}` : "Huippu saavutettu"} />
              <Stat label="Elinajanodote" value={Math.round(derived.lifeExpectancy) + " v"} />
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              {s.nation ? (
                <div className="paper" style={{ padding: "6px 12px", fontSize: 13, background: "rgba(201,162,39,0.1)", color: "#cfd6de" }}>
                  {s.nation.icon} {s.nation.name} <span style={{ color: "#93a3ba" }}>· kansallisvaltio</span>
                </div>
              ) : (
              ...
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setNationOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 620, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 6px", color: "#f0e8d2" }}>🏳️ Muodosta kansallisvaltio</h2>
                  ...
                  <p style={{ fontSize: 13, color: "#93a3ba", margin: "0 0 12px" }}>{s.civ.icon} {s.civ.name} voi jatkaa yhtenä historiallisista kansallisvaltioistaan. Kukin tuo oman tapahtumakirjonsa perussimulaation päälle. Peruspeli pysyy täysin pelattavana ilman näitä — tämä on valinnainen syvennys.</p>
                  {NATION_BRANCHES[s.civ.key].map((n) => {
                    ...
                    return (
                      <div key={n.key} className="civcard" style={{ width: "100%", marginBottom: 8, display: "block" }}>
                        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 700, color: "#f0e8d2" }}>{n.icon} {n.name}</div>
                        <div style={{ fontSize: 11.5, color: "#c9a227", fontWeight: 600, margin: "2px 0 4px" }}>{n.trait}</div>
                        <div style={{ fontSize: 11.5, color: "#cfd6de", marginBottom: 6 }}>{n.events.length} omaa historiallista tapahtumaa: {n.events.map((e) => e.name).join(" · ")}</div>
                        {owned ? (
                          <button className="btn" style={{ padding: "8px 16px", fontSize: 13 }} onClick={() => becomeNation(n)}>Jatka {n.name}na ▸</button>
                        ) : (
                  ...
                  <div style={{ fontSize: 10.5, color: "#93a3ba", marginTop: 6 }}>Demo-osto: tallentuu vain tähän selaimeen, ei oikeaa veloitusta. Oikeassa julkaisussa tämä kytkettäisiin sovelluskaupan maksuun.</div>
                </div>
              </div>
            )}
            ...
            {metricsOpen && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setMetricsOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 560, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 6px", color: "#f0e8d2" }}>📈 Yhteiskuntamittarit</h2>
                  ...
                  <p style={{ fontSize: 13, color: "#93a3ba", margin: "0 0 12px" }}>Mittarit mukailevat oikeita yhteiskuntatieteen tunnuslukuja ja avautuvat sitä mukaa kun vastaava mittausmenetelmä syntyi historiassa. Kunkin ℹ️ kertoo, miten oikea mittari lasketaan.</p>
                  {METRICS.map((m) => {
                    ...
                    return (
                      <div key={m.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 4px", borderBottom: "1px dotted #263a54" }}>
                        <div>
                          <span style={{ fontSize: 14, color: "#cfd6de" }}>{m.icon} {m.name}</span>
                          {unlocked && <InfoButton title={m.infoTitle} text={m.infoText} />}
                          {!unlocked && <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Avautuu aikakaudella: {ERAS[m.unlockEra].name}</div>}
                        </div>
                        {unlocked && (
            ...
            {historyOpen && (() => {
              ...
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setHistoryOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 640, width: "100%", maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 6px", color: "#f0e8d2" }}>📖 {s.villageName ? `${s.villageName}n historia` : "Yhteisöni historia"}</h2>
                  ...
                  <p style={{ fontSize: 12.5, color: "#93a3ba", margin: "0 0 12px" }}>Tämä näkymä kokoaa yhteisösi koko tarinan — hyvä pohja esitellä muille, mitä valintoja teit ja miksi.</p>

                  <div style={{ background: "#12233a", border: "1px solid #2a3d57", borderRadius: 8, padding: "10px 12px", marginBottom: 12 }}>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 700, color: "#f0e8d2" }}>{s.civ.icon} {s.civ.name}{s.nation && <> → {s.nation.icon} {s.nation.name}</>}</div>
                    ...
                    <div style={{ fontSize: 12.5, color: "#cfd6de", marginTop: 4 }}>Nyt: vuosi {s.year} · {era.name} · {fmt(pop)} asukasta · {govByKey(s.gov).icon} {govByKey(s.gov).name}</div>
                  </div>

                  <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: "#93a3ba", marginBottom: 6 }}>Aikajana — keskeiset käännekohdat</div>
                  ...
                  <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: "#93a3ba", marginBottom: 6 }}>Sivilisaatioindeksi nyt: {fmt(score.total)}</div>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, marginBottom: 14 }}>
                    <tbody>
                      {Object.entries(score.parts).map(([k, v]) => (
                        <tr key={k} style={{ borderBottom: "1px dotted #263a54" }}>
                          <td style={{ padding: "3px 4px", color: "#cfd6de" }}>{k}</td>
                  ...
                  <div style={{ background: "rgba(201,162,39,0.08)", border: "1px dashed #c9a22766", borderRadius: 6, padding: "10px 12px", marginBottom: 14, fontSize: 13, color: "#cfd6de" }}>
                    <b style={{ color: "#f0e8d2" }}>💡 Pohdittavaksi esittelyä varten:</b>
                  ...
                  <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: "#93a3ba", marginBottom: 6 }}>Omat huomiosi (näkyy vain sinulle, tällä laitteella)</div>
                  <textarea
                    value={studentNotes}
            ...
            {pyramidOpen && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setPyramidOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 560, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 10px", color: "#f0e8d2" }}>👪 Väestön jakauma</h2>
                  ...
                  <div style={{ display: "flex", gap: 16, marginBottom: 14, flexWrap: "wrap" }}>
                    <div><div style={{ fontSize: 11, color: "#93a3ba", textTransform: "uppercase" }}>Keski-ikä</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 22, fontWeight: 700, color: "#e9e2cf" }}>{Math.round(derived.avgAge)} v</div></div>
                    <div><div style={{ fontSize: 11, color: "#93a3ba", textTransform: "uppercase" }}>Huoltosuhde</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 22, fontWeight: 700, color: "#e9e2cf" }}>{Math.round(derived.depRatio)}</div><div style={{ fontSize: 10.5, color: "#93a3ba" }}>lasta+vanhusta / 100 työikäistä</div></div>
                  </div>
                  {[
                    { label: "👶 Lapset", n: s.children, color: "#c9a227", role: era.roles.children },
                    { label: "💪 Työikäiset", n: s.workers, color: "#7fbf8f", role: era.roles.youth },
                    { label: "👴 Vanhukset", n: s.elderly, color: "#93a3ba", role: era.roles.elderly },
                  ].map((g) => (
                    <div key={g.label} style={{ marginBottom: 12 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#93a3ba" }}>{era.name}: {g.role}</div>
                    </div>
                  ))}
            ...
            {govOpen && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setGovOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 620, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 10px", color: "#f0e8d2" }}>🏛️ Hallintomuoto</h2>
                  ...
                  <p style={{ fontSize: 13, color: "#93a3ba", margin: "0 0 12px" }}>Itsevaltaisilta hallinnoilta puuttuu legitimiteetti — ne tuottavat tehokkaasti mutta kaatuvat herkästi, jos kansa on tyytymätön. Demokratia ja tasavalta ovat hitaampia mutta vakaampia. Teokratia on oma haaransa: vahva yhteisöllisyys, hitaampi tiede.</p>
                  {GOVERNMENTS.map((g) => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 700, color: "#f0e8d2" }}>{g.icon} {g.name} {active && "· nykyinen"}</div>
                        <div style={{ fontSize: 11.5, color: "#c9a227", fontWeight: 600, margin: "2px 0 4px" }}>{g.axis}</div>
                        <div style={{ fontSize: 12, color: "#cfd6de", marginBottom: 4 }}>{g.desc}</div>
                        ...
                        {!unlocked && <div style={{ fontSize: 11, color: "#c96a5a", marginTop: 4 }}>🔒 {govRequirementText(g)}</div>}
                      </button>
                    );
                  })}
                </div>
            ...
            {derived.skillPenalty < 1 && (
              <div style={{ background: "rgba(201,162,39,0.1)", border: "1px solid #c9a22766", borderRadius: 6, padding: "8px 14px", marginBottom: 12, fontSize: 14, color: "#cfd6de" }}>
                ⚠️ <b style={{ color: "#f0e8d2" }}>Osaamispula:</b> {era.name} aikakausi vaatisi vähintään {fmt(eraMinPop)} ihmistä täyteen erikoistumiseen. Tuottavuus nyt {Math.round(derived.skillPenalty * 100)} %.
              </div>
            )}
            ...
            {s.report && (
              ...
              >
                <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, fontWeight: 700, color: "#f0e8d2" }}>📜 Kertomus vuosilta {s.report.period}</span>
                <span style={{ fontSize: 12, color: "#93a3ba", marginLeft: 8 }}>— {s.report.eventName} · lue tarina ▸</span>
              </button>
            )}
            ...
            {reportOpen && s.report && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20, padding: 12 }} onClick={() => setReportOpen(false)}>
                <div className="modal-panel" style={{ maxWidth: 640, width: "100%", maxHeight: "88vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, margin: "0 0 10px", color: "#f0e8d2" }}>📜 Kertomus vuosilta {s.report.period}</h2>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 16, alignItems: "start" }}>
                    <div>
                      <div style={{ background: "#12233a", border: "1px solid #2a3d57", borderRadius: 8, padding: "14px 16px", marginBottom: 10 }}>
                        <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: "#c9a227", marginBottom: 6 }}>{STORY_LABELS[s.report.eraIdx] ?? STORY_LABELS[0]}</div>
                      ...
                      <div style={{ background: "rgba(201,162,39,0.08)", border: "1px dashed #c9a22766", borderRadius: 6, padding: "8px 12px", fontSize: 13, color: "#cfd6de" }}>
                        💡 <b style={{ color: "#f0e8d2" }}>Tiesitkö?</b> {s.report.fact}
                      </div>
                    </div>
                    <div>
                      <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: "#93a3ba", marginBottom: 6 }}>Vuosi vuodelta</div>
                      ...
                      <div style={{ fontSize: 10.5, color: "#93a3ba", marginTop: 4 }}>★ = tapahtuma, joka todella vaikutti tämän jakson laskelmiin</div>
                    </div>
                  </div>
                  <details style={{ marginTop: 10 }}>
                    <summary style={{ cursor: "pointer", fontSize: 13, color: "#c9a227", fontWeight: 600 }}>Näytä luvut ▸</summary>
                ...
                {(() => {
                  ...
                  <div key={key} style={{ marginBottom: 13 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 14, marginBottom: 5, color: "#e9e2cf" }}>
                      <span>{icon} {name}</span>
                      <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 600, color: "#c9a227" }}>{alloc[key]} % · {fmt((s.workers * alloc[key]) / 100)} hlö</span>
                    </div>
                        ...
                        {s.era >= 1 ? (
                          <>
                            ...
                            {s.era >= 2 ? (
                              ...
                              <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Erikoislääkärit avautuvat Teollisesta aikakaudesta.</div>
                            )}
                          </>
                        ) : (
                          <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Terveydenhuollon säädöt avautuvat Varhaisteollisesta aikakaudesta.</div>
                        )}
                      </div>
                    )}
                    ...
                    {key === "research" && (
                      <div style={{ marginTop: 6, paddingLeft: 4, borderLeft: "2px solid #263a54" }}>
                        {s.era >= 1 ? (
                          ...
                          <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Tutkimuksen suuntaaminen avautuu Varhaisteollisesta aikakaudesta.</div>
                        )}
                      </div>
                    )}
                    ...
                    {key === "admin" && (
                      <div style={{ marginTop: 6, paddingLeft: 4, borderLeft: "2px solid #263a54" }}>
                        {s.era >= 2 ? (
                          ...
                          <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Rikostutkijat avautuvat Teollisesta aikakaudesta.</div>
                        )}
                      </div>
                    )}
                    ...
                    {key === "industry" && (
                      <div style={{ marginTop: 6, paddingLeft: 4, borderLeft: "2px solid #263a54" }}>
                        {s.era >= 1 ? (
                          ...
                          <div style={{ fontSize: 10.5, color: "#93a3ba" }}>🔒 Teollisuuden suuntaaminen avautuu Varhaisteollisesta aikakaudesta.</div>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <Section title="Ammattijakauma" open={profOpen} onToggle={() => setProfOpen(!profOpen)}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                  <tbody>
                    ...
                    <tr><td style={{ padding: "6px 4px", color: "#93a3ba" }}>Työvoiman ulkopuolella (lapset & vanhukset)</td>
            ...
            <div style={{ textAlign: "center", margin: "18px 0" }}>
              <button className="btn" style={{ width: "100%", maxWidth: 420 }} onClick={nextTurn} disabled={!!s.gameOver}>Etene 5 vuotta →</button>
            </div>
            ...
            <div className="grid-graphlog">
              <div className="paper" style={{ padding: 10 }}>
                <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, margin: "0 0 4px", color: "#f0e8d2" }}>Väestökehitys</h2>
                <ResponsiveContainer width="100%" height={90}>
                  <LineChart data={s.history} margin={{ top: 5, right: 6, bottom: 0, left: 0 }}>
                    ...
                    <Tooltip formatter={(v) => [fmt(v), "Väestö"]} labelFormatter={(y) => "Vuosi " + y} contentStyle={{ background: "#0d1c30", border: "1px solid #2a3d57", fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "#e9e2cf" }} />
              ...
              <div className="paper" style={{ padding: 10 }}>
                <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, margin: "0 0 4px", color: "#f0e8d2" }}>Aikakirjat</h2>
        ...
        {s.gameOver && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(6,12,22,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10, padding: 12 }}>
            <div className="modal-panel" style={{ maxWidth: 480, width: "100%", textAlign: "center", padding: 24, maxHeight: "90vh", overflowY: "auto" }}>
              ...
              <div style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: 1.5, color: "#93a3ba", marginBottom: 8 }}>Sivilisaatioindeksi</div>
              <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse", marginBottom: 12 }}>
                <tbody>
                  {Object.entries(score.parts).map(([k, v]) => (
                    <tr key={k} style={{ borderBottom: "1px dotted #263a54" }}>
                      <td style={{ textAlign: "left", padding: "3px 4px", color: "#cfd6de" }}>{k}</td>
                      <td style={{ textAlign: "right", fontFamily: "'IBM Plex Mono',monospace", fontWeight: 600, color: "#e9e2cf" }}>{fmt(v)}</td>
                    </tr>
                  ))}
                </tbody>
              ...
              {window.storage && (
                <div style={{ background: "#12233a", border: "1px solid #2a3d57", borderRadius: 8, padding: 12, marginBottom: 12 }}>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 700, marginBottom: 8, color: "#f0e8d2" }}>🏆 Yhteinen tulostaulu</div>
                  {!scoreSaved ? (
                    ...
                    <div style={{ color: "#7fbf8f", fontSize: 14, fontWeight: 600 }}>✓ Tulos tallennettu!</div>
                  )}
                  <div style={{ fontSize: 11, color: "#93a3ba", marginTop: 6 }}>Nimi ja tulos näkyvät kaikille tämän pelin pelaajille.</div>
                      ...
                      <tbody>
                        {board.slice(0, 10).map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px dotted #263a54", background: scoreSaved && r.name === playerName.trim() && r.score === score.total ? "rgba(201,162,39,0.12)" : "transparent" }}>
                            ...
                            <td style={{ textAlign: "left", padding: "3px 4px", color: "#cfd6de" }}>{r.icon} {r.name} <span style={{ color: "#93a3ba", fontSize: 11 }}>({r.civ}{r.win ? " · voitto" : ""})</span></td>
                  ...
                  {board && board.length === 0 && <div style={{ fontSize: 13, color: "#93a3ba", marginTop: 8 }}>Ole ensimmäinen tulostaululla!</div>}
                </div>
              )}

              <p style={{ fontSize: 13, color: "#93a3ba", margin: "0 0 10px" }}>Väestöhuippu: {fmt(Math.max(...s.history.map((h) => h.pop)))} · Vuosia: {s.year}</p>
