...
| Pe 24.7. | Telemetrian runko: paikallinen tapahtumaloki (avatut laatikot, vuorot, sessioaika, saavutettu aikakausi) + "Vie raportti" -nappi opettajanäkymään (JSON/CSV). Viikon commit + zip. | **D5 pohja valmis ✅** *(tehty 11.7. — pelitestaus ja korjaukset (ti–ke) yhä tekemättä, ks. alla)* |
...
| Pe 31.7. | Koulusimulaatio: testaa bundle hitaalla yhteydellä (Chrome DevTools throttling), vanhalla koneella/halvalla Chromebook-resoluutiolla (1366×768), ja ilman verkkoa. Korjaa löydökset. Viikon commit + zip. | **Tehty 13.7. ✅** *(ei Chrome DevTools -selainautomaatiota tässä ympäristössä — mitattu oikeat siirtokoot ja testattu verkkokatko jsdomilla sen sijaan, ks. [KOULUSIMULAATIO.md](KOULUSIMULAATIO.md). Verkkokatko: ei löydöksiä. Fonttien render-blocking lataus: korjattu. Merkittävä löydös — testaajien käyttämä sivu 3,5–4× raskaampi kuin jo olemassa ollut mutta julkaisematon Vite-bundle — **ratkaistu**: Vite-bundle julkaistu `/play/`-osoitteeseen GitHub Pagesiin käyttäjän vahvistuksella, testaajien ja markkinointisivun linkit vaihdettu sinne. 1366×768: koodikatselmus puhtaana, visuaalinen vahvistus jäi auki.)* |
